 ...
 *
 */
public class Node4 extends JFrame
{
	...
	String destination,way,dest,check,path;
	ServerSocket ss4,ss2;
	Socket s4,s5,s6,s2;
	ResultSet rs;
			 ...
			 while(true)
				{
						...
						s4=ss4.accept();
						DataInputStream di=new DataInputStream(s4.getInputStream());					
						display=di.readDouble();
						String check=di.readUTF();
						String dest=di.readUTF();
						...
						//System.out.println(way);
						 ...
						 if(dest.equalsIgnoreCase("Node4"))
					      {
	...
	private void initializeComponent()
	{
		...
		String st[]={"Select","Node1","Node2","Node3","Node5"};
		...
		jTabbedPane1.addTab("Node4", jPanel1);
	...
	//	
		...
		// Node1
		//
		this.setTitle("Node4 - extends JFrame");
		...
		try
			{
				...
				String path="path:Node4";
				time=System.nanoTime();;
				Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
					  Connection con=DriverManager.getConnection("jdbc:odbc:path","sa","");
					  Statement stmt=con.createStatement();
					  stmt.executeUpdate("truncate table path");
				if(destination.equalsIgnoreCase("Node4"))	
						{
